...
let render window candidates selection =
  ...
  let size = { LTerm_geom.rows = LTerm_geom.rows size; cols = LTerm_geom.cols size } in
  ...
  let rect = { LTerm_geom.row1 = 1; col1 = 0; row2 = LTerm_geom.rows size; col2 = LTerm_geom.cols size } in
  let ctx = LTerm_draw.context mat size in
  let ctx = LTerm_draw.sub ctx rect in
  List.fold_left candidates ~init:0 ~f:(fun current candidate ->
  ...
  LTerm.render_update window displayed mat >>= fun () -> LTerm.goto window { LTerm_geom.row = 0; col = 0 }
