...
let update_selection info selection =
  ...
  ( if info.I.changed then info.I.selection <- None
  else
    match selection with
    | None     -> info.I.selection <- None
    | Some dir -> (
        match (dir, info.I.selection) with
        ...
        | Types.Next, Some (row, _) -> info.I.selection <- Some (succ row |> min len, Const.selection_style)
        | Types.Prev, Some (row, _) -> info.I.selection <- Some (pred row |> max 0, Const.selection_style) ) );
  info
